class RMQMAX:  # for MAX queries
    ...
    def buildSparseTable(self):
        ...
        while (1 << j) <= n:
            i = 0
            while (i + (1 << j) - 1) < n:
                self.lookup[i * self.M + j] = max(
                    self.lookup[i * self.M + j - 1],
                    self.lookup[(i + (1 << (j - 1))) * self.M + j - 1],
                )
                i += 1
            j += 1
    ...
    def query(self, L, R):  # returns min on interval [l,r] inclusive
        j = self.log2[R - L + 1]
        return max(
            self.lookup[L * self.M + j], self.lookup[(R - (1 << j) + 1) * self.M + j]
        )
...
class OrderedList(SortedList):  # Codeforces, Ordered Multiset
    ...
    def rangeCountByValue(self, leftVal, rightVal):
        # returns number of items in range [leftVal,rightVal] inclusive
